...
export async function POST(request: Request, context: Params) {
  try {
    ...
    const { error } = await supabase.from("build_group_ratings").upsert(
      { user_id: user.id, build_group_id: groupId, rating },
      { onConflict: "user_id,build_group_id" }
    );
    ...
    const stat = voteStats.get(groupId) ?? { votes: 0, rawAverage: 0 };
