...
export default function NewBattleGamePage() {
  ...
  async function submit(e: React.FormEvent) {
    ...
    setBusy(true);
    setError("");
    const res = await fetch("/api/battle-games", {
      method: "POST",
    ...
    if (data.id) router.push(`/battle-games/${data.id}`);
    else router.push("/battle-games");
    router.refresh();
  }
  ...
  return (
    <main className="p-10 text-white max-w-lg space-y-6">
      ...
      <h1 className="text-2xl font-bold">Create battle game</h1>
